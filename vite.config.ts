...
function trimLogFile(logPath: string, maxSize: number) {
  try {
    ...
    const lines = fs.readFileSync(logPath, "utf-8").split("\n"); const keptLines: string[] = []; let keptBytes = 0;
    for (let i = lines.length - 1; i >= 0; i--) { const lineBytes = Buffer.byteLength(`${lines[i]}\n`, "utf-8"); if (keptBytes + lineBytes > TRIM_TARGET_BYTES) break; keptLines.unshift(lines[i]); keptBytes += lineBytes; }
    fs.writeFileSync(logPath, keptLines.join("\n"), "utf-8");
  } catch { /* ignore trim errors */ }
}
function writeToLogFile(source: LogSource, entries: unknown[]) {
  if (!entries.length) return; ensureLogDir(); const logPath = path.join(LOG_DIR, `${source}.log`);
...
function vitePluginManusDebugCollector(): Plugin {
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use("/__manus__/debug-collector.js", (req: any, res: any, next: any) => {
        if (req.method !== "GET" && req.method !== "HEAD") return next();
        const collectorPath = path.join(PROJECT_ROOT, "client", "src", "dev", "debug-collector.js");
        res.setHeader("Content-Type", "application/javascript; charset=utf-8"); res.setHeader("Cache-Control", "no-store");
        if (req.method === "HEAD") return res.end(); fs.createReadStream(collectorPath).on("error", next).pipe(res);
      });
      server.middlewares.use("/__manus__/logs", (req: any, res: any, next: any) => {
        ...
        let body = ""; req.on("data", (chunk: unknown) => { body += String(chunk); });
        req.on("end", () => { try { handlePayload(JSON.parse(body)); } catch (e) { res.writeHead(400, { "Content-Type": "application/json" }); res.end(JSON.stringify({ success: false, error: String(e) })); } });
      });
    },
  };
}
const plugins: Plugin[] = [react(), tailwindcss(), jsxLocPlugin(), vitePluginManusRuntime()];
if (process.env.NODE_ENV === "development") plugins.push(vitePluginManusDebugCollector());
