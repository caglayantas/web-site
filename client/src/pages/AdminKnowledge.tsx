...
type KnowledgeForm = { id?: number; slug: string; category: string; title: string; excerpt: string; coverImage: string; body: string; seoTitle: string; seoDescription: string; categoryEn: string; titleEn: string; excerptEn: string; bodyEn: string; seoTitleEn: string; seoDescriptionEn: string; publishedAt: string; status: "draft" | "published"; featured: boolean; sortOrder: number };
const emptyForm: KnowledgeForm = { slug: "", category: "Marin elektrik", title: "", excerpt: "", coverImage: "", body: "## Teknik not\n\nİçeriğinizi markdown ile yazın.\n\n- Kontrol başlığı\n- Uygulama notu\n- Bakım önerisi", seoTitle: "", seoDescription: "", categoryEn: "", titleEn: "", excerptEn: "", bodyEn: "", seoTitleEn: "", seoDescriptionEn: "", publishedAt: new Date().toISOString().slice(0, 16), status: "draft", featured: false, sortOrder: 0 };
...
export default function AdminKnowledge() {
  ...
  const editPost = (post: KnowledgePostRow) => setForm({ id: post.id, slug: post.slug, category: post.category, title: post.title, excerpt: post.excerpt, coverImage: post.coverImage ?? "", body: post.body, seoTitle: post.seoTitle ?? "", seoDescription: post.seoDescription ?? "", categoryEn: post.categoryEn, titleEn: post.titleEn, excerptEn: post.excerptEn, bodyEn: post.bodyEn, seoTitleEn: post.seoTitleEn ?? "", seoDescriptionEn: post.seoDescriptionEn ?? "", publishedAt: post.publishedAt ? new Date(post.publishedAt).toISOString().slice(0, 16) : "", status: post.status, featured: post.featured, sortOrder: post.sortOrder });
