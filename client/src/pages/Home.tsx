  ...
  return (
    <div className="home-page-compact">
      ...
      <section id="teknik-bilgiler" className="section journal-section">
        ...
        <div className="journal-grid">{knowledgeLoading ? [0, 1, 2].map((index) => <div className="home-content-skeleton home-content-skeleton--journal" key={`journal-skeleton-${index}`} aria-hidden="true" />) : technicalCards.length > 0 ? technicalCards.map((card, index) => <a key={card.slug} href={`/teknik-bilgiler/${card.slug}`} className={`journal-card${index === 0 ? " journal-card--feature" : ""}`}>{card.coverImage && <img className="journal-card__cover" src={card.coverImage} alt="" loading="lazy" decoding="async" />}<div className="journal-card__overlay"><span>{card.category}</span><h3>{card.title}</h3><p>{card.excerpt}</p></div><FileText size={19} /></a>) : <p className="home-content-empty">Henüz yayınlanmış teknik bilgi bulunmuyor.</p>}</div>
      </section>
