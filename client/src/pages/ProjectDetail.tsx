...
export default function ProjectDetail() {
  ...
  return <div className="corporate-page project-post-page"><article className="project-post"><Link href={toPath("/projeler")} className="text-link text-link--dark"><ArrowLeft size={16} /> {t.back}</Link><header className="project-post__header"><p className="eyebrow">{project.label}</p><h1>{project.title}</h1><p>{project.detail}</p></header><BeforeAfterSlider before={project.beforeImage} after={project.afterImage} beforeAlt={`${project.title} ${t.beforePhoto}`} afterAlt={`${project.title} ${t.afterPhoto}`} label={project.title} fit={project.photoFit} />{project.galleryImages.length > 0 && <div className="project-post__gallery">{project.galleryImages.map((url, index) => project.photoFit === "contain" ? <div className="project-post__gallery-item" key={url}><img className="project-post__gallery-blur" src={url} alt="" aria-hidden="true" /><img className="project-post__gallery-photo" src={url} alt={`${project.title} — ${t.photo} ${index + 1}`} loading="lazy" decoding="async" /></div> : <img key={url} src={url} alt={`${project.title} — ${t.photo} ${index + 1}`} loading="lazy" decoding="async" />)}</div>}{(() => { const embed = getVideoEmbed(project.videoUrl); return embed && <div className={`project-post__video project-post__video--${embed.platform}`}><iframe src={embed.embedUrl} title={`${project.title} — video`} loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /></div>; })()}<div className="project-post__facts"><section><h2>{t.scope}</h2><p>{project.scope || t.scopeFallback}</p></section><section><h2>{t.systems}</h2><p>{project.systems || t.systemsFallback}</p></section><section><h2>{t.results}</h2><p>{project.results || t.resultsFallback}</p></section></div><footer className="project-post__footer"><Link href={`${toPath("/iletisim")}?kategori=${encodeURIComponent(project.label)}`} className="button button--gold">{t.cta} <ArrowUpRight size={16} /></Link><a href={toPath(`/projeler/${project.slug}`)} className="text-link text-link--dark">{t.permalink}</a></footer></article></div>;
}
